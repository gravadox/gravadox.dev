...
export default function CVEditor({lang}:{lang: "EN" | "DE"}) {
  ...
  useEffect(() => {
    getCv(lang).then((data: RawCV | null) => {
      if (!data) return
  ...
  return (
    <div className="flex gap-10 p-10 w-full">
      <form action={submitSaveCV} className="flex flex-col gap-8 p-6 border rounded-lg w-xl">
        <input type="text" name="lang" defaultValue={lang} className="hidden" required ></input>
        {/* GENERAL */}
