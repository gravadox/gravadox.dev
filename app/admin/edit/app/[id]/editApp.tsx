...
import { BlocksEditor, EditorBlock, makeId } from "@/components/admin/editor/blocks-editor"
...
export default function EditApp({ appId }: EditAppProps) {
  ...
  useEffect(() => {
    getAppById(appId).then(app => {
      if (!app) { toast("App not found"); router.push("/admin"); return }
      setTitle(app.title)
      setDescription(app.description || "")
      setBanner(app.banner || "")
      setIcon(app.icon || "")
      setGithub(app.github || "")
      setVersion(app.version || "")
      setSlug(app.slug)
      setTags(app.tags.join(", "))
      setPublishAt(app.publishAt ? new Date(app.publishAt).toISOString().slice(0, 16) : "")
      setPinned(app.pinned?.toString() || "")
      setDownloads(app.downloads.map(d => ({ name: d.name, link: d.link })))
      setBlocks(app.blocks.map(b => ({
        _id: makeId(),
        type: b.type,
        data: b.data && typeof b.data === "object" && b.data !== null
          ? Object.fromEntries(Object.entries(b.data).map(([k, v]) => [k, String(v ?? "")]))
          : {},
      })))
    }).catch(() => toast("Failed to load app")).finally(() => setLoading(false))
  }, [appId, router])
