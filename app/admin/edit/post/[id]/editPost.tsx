...
import { getPostById, updatePost } from "@/actions/admin"
...
export default function EditPost({ postId }: EditPostProps) {
  ...
  useEffect(() => {
    getPostById(postId).then(post => {
      if (!post) { toast("Post not found"); router.push("/admin"); return }
      setTitle(post.title)
      ...
      setPinned(post.pinned?.toString() || "")
    ...
    }).catch(() => toast("Failed to load post")).finally(() => setLoading(false))
  }, [postId, router])
